...
class HistoricalCanadiansParser:
    ...
    def cache_time_spans(self):
        ...
        # Note: RDF uses E52_Time-Span with hyphen, not underscore
        E52_TimeSpan = URIRef("http://www.cidoc-crm.org/cidoc-crm/E52_Time-Span")
        time_span_uris = set(self.graph.subjects(RDF.type, E52_TimeSpan))

        for ts_uri in tqdm(time_span_uris, desc="Time-spans"):
            time_span = {}

            # Get human-readable date
            for date_val in self.graph.objects(ts_uri, CRM.P82_at_some_time_within):
                time_span['display'] = str(date_val)

            # Get ISO begin date
            for begin_val in self.graph.objects(ts_uri, CRM.P82a_begin_of_the_begin):
                time_span['begin'] = str(begin_val)

            # Get ISO end date
            for end_val in self.graph.objects(ts_uri, CRM.P82b_end_of_the_end):
    ...
    def cache_places(self):
        ...
        place_uris = set(self.graph.subjects(RDF.type, CRM.E53_Place))
        ...
        for place_uri in tqdm(place_uris, desc="Places"):
            ...
            for geonames_uri in self.graph.objects(place_uri, CRM.P89_falls_within):
                geonames_id = extract_geonames_id(str(geonames_uri))
                if geonames_id:
                    place['geonamesId'] = geonames_id

            # Get coordinates (P168_place_is_defined_by)
            for coords in self.graph.objects(place_uri, CRM.P168_place_is_defined_by):
    ...
    def extract_persons(self):
        ...
        person_uris = set(self.graph.subjects(RDF.type, CRM.E21_Person))
    ...
    def extract_birth_events(self):
        ...
        birth_uris = set(self.graph.subjects(RDF.type, CRM.E67_Birth))
        ...
        for birth_uri in tqdm(birth_uris, desc="Processing births"):
            ...
            person_uri = None
            for p in self.graph.objects(birth_uri, CRM.P98_brought_into_life):
                person_uri = str(p)
                break
            ...
            birth_places = []
            for place_uri in self.graph.objects(birth_uri, CRM.P7_took_place_at):
                place_str = str(place_uri)
            ...
            for ts_uri in self.graph.objects(birth_uri, CRM.P4_has_time_span):
                ts_str = str(ts_uri)
            ...
            # Get parents
            for mother_uri in self.graph.objects(birth_uri, CRM.P96_by_mother):
                mother_str = str(mother_uri)
                if mother_str in self.persons:
                    birth_event['motherId'] = self.persons[mother_str]['personId']

            for father_uri in self.graph.objects(birth_uri, CRM.P97_from_father):
    ...
    def extract_death_events(self):
        ...
        death_uris = set(self.graph.subjects(RDF.type, CRM.E69_Death))
        ...
        for death_uri in tqdm(death_uris, desc="Processing deaths"):
            ...
            person_uri = None
            for p in self.graph.objects(death_uri, CRM.P100_was_death_of):
                person_uri = str(p)
                break
            ...
            death_places = []
            for place_uri in self.graph.objects(death_uri, CRM.P7_took_place_at):
                place_str = str(place_uri)
            ...
            for ts_uri in self.graph.objects(death_uri, CRM.P4_has_time_span):
                ts_str = str(ts_uri)
                if ts_str in self.time_spans:
                    ts_data = self.time_spans[ts_str]
    ...
    def extract_marriages(self):
        ...
        marriage_uris = set(self.graph.subjects(RDF.type, CRM.E85_Joining))
        ...
        for marriage_uri in tqdm(marriage_uris, desc="Processing marriages"):
            ...
            spouses = []
            for person_uri in self.graph.objects(marriage_uri, CRM.P143_joined):
                person_str = str(person_uri)
            ...
            # Get marriage date
            marriage_date = None
            for ts_uri in self.graph.objects(marriage_uri, CRM.P4_has_time_span):
                ts_str = str(ts_uri)
                if ts_str in self.time_spans:
